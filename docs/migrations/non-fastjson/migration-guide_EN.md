...
For example, the current recommended release is `0.6.161-non-fastjson.2`. The suffix is part of a normal release version. It is not a classifier and does not make the artifact a SNAPSHOT. Maven considers `0.6.161-non-fastjson.2` newer than `0.6.161`, but applications should still select the intended version explicitly. Unqualified `0.6.x` releases will remain available during the transition.

An application must select exactly one line. Do not put `0.6.161` and `0.6.161-non-fastjson.2` in the same dependency graph: Maven will select only one of them, while downstream code may have been compiled against the other API.
...
The resulting file is `aliyun-log-0.6.161-non-fastjson.2-jar-with-dependencies.jar`.
...
For a temporary rollback, restore the paired unqualified version—for example, change `0.6.161-non-fastjson.2` back to `0.6.161`. Do not retain both versions.
